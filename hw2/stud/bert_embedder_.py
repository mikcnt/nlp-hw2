...
class BERTEmbedder(pl.LightningModule):
    ...
    def _tokenize_sentence(self, sentence: List[str]):
        ...
        for word in sentence:
            ...
            to_merge_wordpiece.append(
                [
                    i
                    for i in range(
                        len(encoded_sentence) - 1,
                        len(encoded_sentence) + len(encoded_word) - 1,
                    )
                ]
            )
            encoded_sentence.extend(
                self.bert_tokenizer.convert_tokens_to_ids(encoded_word)
            )
    ...
    def _merge_embeddings(
        self, aggregated_layers: List[List[float]], to_merge_wordpieces: List[List[int]]
    ):
        merged_output = []
        # first we remove the [CLS] and [SEP] tokens from the output embeddings
        aggregated_layers = aggregated_layers[:, 1:-1, :]
        for embeddings, sentence_to_merge_wordpieces in zip(
            aggregated_layers, to_merge_wordpieces
        ):
            sentence_output = []
